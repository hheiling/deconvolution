...
5. Assign a constant dispersion parameter value for all genes. 

As a result of this step, we should have the negative binomial mean parameters for each gene cluster and a constant dispersion parameter for all genes.
...
The sum of the E elements of the vector $\mu_g$ should equal the negative binomial mean parameter for the gene cluster. 

$(\sum_{e=1}^E x_{ge})\beta_{gi}$ should equal the negative binomial mean parameter for each isoform $i$ within the gene cluster ($\mu_{gi}$). 

$\sum_{i=1}^I \mu_{gi}$ should also equal the negative binomial mean paramter for the gene cluster. This will be true as long as we simulate isoform negative binomial parameters as described in the "Gene Cluster to Isoform Parameters" section. 

### Details to Simulate Exon Set Counts

1. If we simulate isoform negative binomial parameters as described in the "Gene Cluster to Isoform Parameters" section, we can calculate the $\beta_g$ parameters by taking each isoform mean parameter and dividing by the sum of the corresponding column in the $X_g$ effective length design matrix.
