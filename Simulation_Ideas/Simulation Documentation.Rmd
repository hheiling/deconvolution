...
counts_output(exonInfo_1000 = iso_exon_1000, exonInfo_other = iso_exon_other,
              ...
              folder = "Simulated_Output/Pure_Sample_Counts", seed = seeds[11])
```
...
The mixture files will be created during the simulation process and then deleted to save space. However, here is an example of mixture file creation for demonstration.

In the simulation, the number of mixture replicates per probabiltiy combination will be 20. I reduced number of 5 is used here for illustration.

```{r}
# Specify number of mixture replicates per probability combination
mix_rep = 5
rep_labels = str_c("0",1:mix_rep)
...
```{r, eval=FALSE}
# Simulating mixture counts for first probability combination:
mix_creation2(set_mixSim = set_mixSim, out_folder = "Simulated_Output/Mixture Sample Counts",
             file_labels = mix_labels[,1], total_cts = total_cts_mix[,p], probs = p_combos[p,], 
             seed = seeds[13])
```
